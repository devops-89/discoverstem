  ...
  return (
    <Dialog open={open} onClose={onClose}  disableScrollLock maxWidth={false} slotProps={{  paper:{sx: { width: "1160px", maxWidth: "95vw", maxHeight: "90vh", borderRadius: "29px", backgroundColor: "#F7F7F7", overflowY: "auto", position: "relative" }} }}>
      ...
      <Box sx={{ p: { xs: 3, md: "30px" } }}>
        ...
        <Typography sx={{ fontFamily: "Poppins", fontWeight: 600, fontSize: { xs: "22px", md: "25.6px" }, lineHeight: "29.4px", letterSpacing: "-0.64px", color: "#000000", mb: 1 }}>
          {data.name}
        </Typography>
        <Typography sx={{ fontFamily: "Poppins", fontWeight: 400, fontSize: "14px", lineHeight: "20px", color: "rgba(0,0,0,0.75)", mb: 3 }}>
          {data.degree}{data.org ? ` · ${data.org}` : ""}
        </Typography>
        {descriptions.map((paragraph, index) => (
          <Typography key={index} sx={{ fontFamily: "Poppins", fontWeight: 400, fontSize: "16px", lineHeight: "20px", letterSpacing: "-0.03em", color: "#777777", mb: index === descriptions.length - 1 ? 0 : 1.2 }}>
            {paragraph}
          </Typography>
        ))}
        <Box sx={{ clear: "both" }} />
