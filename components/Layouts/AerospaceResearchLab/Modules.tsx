...
import { ResearchModulesSectionData } from "@/utils/Types";
...
export default function ResearchModulesSection({
  data,
}: ResearchModulesSectionProps) {
  const [page, setPage] = useState(0);

  const cardsPerPage = 8;
  const totalPages = Math.ceil(data.modules.length / cardsPerPage);

  const visibleModules = data.modules.slice(
    page * cardsPerPage,
        ...
        {Array.from({ length: totalPages }).map((_, index) => (
          <Box
            ...
            sx={{
              ...
              height: "10px",
              borderRadius: "50%",
              bgcolor: page === index ? "#000000" : "#D9D9D9",
              cursor: "pointer",
            }}
          />
        ))}
